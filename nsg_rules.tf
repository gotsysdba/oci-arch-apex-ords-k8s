...
locals {
  ...
  worker_internet_local_rules = flatten([for service, ips in local.oke_worker_egress_cidr : [
    for ip in ips :
    { hostname = service, cidr = ip }]
  ])

  worker_internet_cidr_rules = var.worker_nsg_lockdown ? {
...
locals {
  ...
  all_rules = { for x, y in merge(
    ...
    merge([for p, _ in local.oke_worker_egress_cidr : { for k, v in local.worker_internet_cidr_rules : k => merge(v, { "nsg_id" = oci_core_network_security_group.oke_workers_lockdown[p].id }) if v.host == p }]...)
    ) : x => merge(y, {
      description               = x
